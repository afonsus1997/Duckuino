...
void runLine(){
  ...
  String x = buf;
  //oled.println(String(buf));
  Serial.print("Space: "); Serial.println(x);
 ...
 // for (int l = 0; l < space; l++) { oled.print(buf[l]);  }
  ...
  if (space == -1) runCommand(0, bufSize);
	  ...
	  //else if(strcmp("DELAY",arg)) {
		  Serial.print("Delay buffer: "); Serial.println();
		  oled.println("DELAY"); oled.println(arg);
		  delay(delay_val);
		//delay(getInt(buf, space)); 
		//delay(String(buf).substring(space + 1, bufSize).toInt());
...
#endif
	// oled.println(equalsBuffer(s, e, "GUI"));
